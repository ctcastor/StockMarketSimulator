...
public class PortfolioDriver {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		Portfolio myPortfolio = initializePortfolio(keyboard);
		myPortfolio.displayPositions();
	
		DecimalFormat formatter = new DecimalFormat("#0.00");
		System.out.println("\nCash Balance: " + "$" + formatter.format(myPortfolio.getCashBalance()) + "\t% Portfolio: " + formatter.format((myPortfolio.getCashBalance() / myPortfolio.calcTotalAccountValue())*100));
		...
		while (choice != 3) {
			if (choice == 1) {
				...
				myPortfolio.editPosition(choice2, numberOfShares);
				myPortfolio.displayPositions();
				System.out.println("\nCash Balance: " + formatter.format(myPortfolio.getCashBalance()) + "\t% Portfolio: " + formatter.format((myPortfolio.getCashBalance() / myPortfolio.calcTotalAccountValue())*100));	
			}
			else if (choice == 2) {
				myPortfolio.updateCurrentPrices();
				myPortfolio.displayPositions();
				System.out.println("\nCash Balance: " + formatter.format(myPortfolio.getCashBalance()) + "\t% Portfolio: " + formatter.format((myPortfolio.getCashBalance() / myPortfolio.calcTotalAccountValue())*100));
			}
			else {
	...
	public static Portfolio initializePortfolio(Scanner keyboard) {
		...
		if(savedPortfolio.length() == 0 || savedCashBalance.length() == 0) {
			portfolio = initializeNewPortfolio(keyboard);
		} 
		else {
			try {
	...
	public static Portfolio initializeNewPortfolio(Scanner keyboard) {
		Portfolio portfolio = new Portfolio();
		...
		Stock stock1 = new Stock(symbol[0], currentShares[0], currentPrices[0]);
		Stock stock2 = new Stock(symbol[1], currentShares[1], currentPrices[1]);
		Stock stock3 = new Stock(symbol[2], currentShares[2], currentPrices[2]);
		Stock stock4 = new Stock(symbol[3], currentShares[3], currentPrices[3]);
